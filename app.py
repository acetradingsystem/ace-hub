...
# D1 scanners: valid after 16:00 ET on trading days
d1_active = is_trading_day and hour >= 16
# H1 scanners: valid at 10:31 ET on trading days
h1_active = is_trading_day and (hour > 10 or (hour == 10 and minute >= 31))

def dot(color):
    return f'<span class="ace-dot {color}"></span>'

d1_dot  = "dot-green" if d1_active  else "dot-dim"
h1_dot  = "dot-blue"  if h1_active  else "dot-dim"
...
<div class="ace-wrapper">
  ...
  <div class="ace-status-bar">
    <div class="ace-status-pill">{dot(d1_dot)} {d1_text}</div>
    <div class="ace-status-pill">{dot(h1_dot)} {h1_text}</div>
    <div class="ace-status-pill">{dot("dot-dim")} Excludes: Mining · Energy · Gold · Materials · Utilities · Real Estate</div>
  </div>
